...
module.exports = class PluginManager extends Manager {
        ...
        for (const cmd of store) {
            if (cmd.keywords.includes(keywords[index])) {
                if (index < keywords.length - 1)
                    return this.findCommandsByClasses(
                        classes,
                        cmd._.childs,
                        index + 1
                    )
    ...
    async loadAllPlugins(plgDir = kb2abot.config.DIR.PLUGIN, silent = true) {
        ...
        for (const file of files) {
            ...
            try {
                this.add(
                    await this.loadCommand(path.join(file, '..', cfg.entry))
                )
            } catch (e) {
                log(
                    e.message,
                    path.join(
                        ...
                        new Date(Date.now()).toISOString().replace(/:/g, '.') +
                            '.log'
                    )
                )
                !silent &&
                    console.newLogger.warn(`PLUGIN ${cfg.name} --> ${e.stack}`)
            } finally {
                !silent &&
            ...
            try {
                ...
                if (e.code === 'MODULE_NOT_FOUND') {
                    ...
                    for (const dep of filteredModules) {
                        try {
                            ...
                            console.newLogger.error(
                                `Dependency "${dep}" khong co tren npm!`
                            )
                            throw new Error(
                    ...
                    try {
                        ...
                        console.newLogger.error(
                            `Gap loi khi chay > npm i cac dependencie(s): ${commaJoin}!`
                        )
                    }
                }
            }
