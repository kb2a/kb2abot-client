...
module.exports = class Weather extends Command {
	constructor() {
		super({
			...
			help:
				"[--set=<locationName> | -s <locationName>] [--location=<locationName> | -l <locationName>]",
	...
	execute(args, api, parent, mssg, group) {
		...
		if (hasHelpParam(args)) {
			if (!location) {
				if (!group.location) {
					const replyMsg =
						'Bạn chưa đặt vị trí mặc định cho group, hãy xài lệnh: /weather --set "[location]"';
					api.sendMessage(replyMsg, mssg.threadID);
					log(
						{
							text: replyMsg,
			...
			axios
				...
				.then(response => {
					...
					const replyMsg = `Name: ${name}${os.EOL}Weather: ${
						weather[0].main
