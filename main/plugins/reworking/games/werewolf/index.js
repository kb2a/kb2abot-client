...
import {cloneObject} from "../../../../helper/helperMaSoi.js";
...
class Werewolf extends Game {
	constructor(config) {
		...
		this.prepairs = ["rollup", "start", "play"];
	}
	...
	update(body, api, parent, mssg, group, groupManager) {
		if (this.prepairs[0] == "rollup") {
			if (body.toLowerCase().indexOf("meplay") != -1) {
				...
				api.sendMessage(
					`Số người chơi sẵn sàng: ${this.playerManager.getLength()}/7`,
					mssg.threadID
				);
			}
			if (this.playerManager.getLength() == 7) {
				// start gaming . . .
				...
				api.sendMessage(
					"Bạn có muốn thêm param nào cho game Werewolf không?",
					mssg.threadID
				);
			}
			return;
		...
		if (this.prepairs[0] == "start" && mssg.senderID == this.owner) {
			let players = "";
			for (const player of this.playerManager.items) {
				if (player.id != this.owner) players += `${player.index} `;
			}
			// for (let i = 2; i <= 7; i++) {
			// 	players += `${i} `;
			// }
			const query = `werewolf --caller ${
				this.playerManager.find("id", this.owner, true).index
			} --tag ${mssg.threadID} start ${body} ${players}`;
		...
		if (this.playerManager.find("id", mssg.senderID) != -1) {
			const query = `werewolf --caller ${
				this.playerManager.find("id", mssg.senderID, true).index
			} --tag ${mssg.threadID} ${body}`;
			exec(query, (err, stdout) => {
				handleGameOutput(api, mssg, this, err, stdout);
			});
			console.log(query);
