...
class ZingMp3 extends Command {
	constructor() {
		super({
			...
			help:
				"[--play=<musicID> | -p <musicID>] [--search=<musicName> | -s <musicName>] [--autodelete=<boolean>]",
	...
	getMusicInfo(id) {
		...
		if (index != -1) return this.musicsInfo[index];
		else return null;
	}
			...
			axios
				.get(
					`https://zingmp3.vn/api/search/multi?q=${encodeURI(
						song
					)}&ctime=1593161576&sig=${this.MP3_SIG}&api_key=${
						this.MP3_KEY
					}`
				)
				.then(response => {
					...
					const length =
						songData.items.length > 10 ? 10 : songData.items.length;
					...
					for (let i = 0; i < length; i++) {
						...
						replyMsg =
							replyMsg +
							`${i +
								1}. Title: ${title}, artist: ${artists_names}, id: ${id}${
								os.EOL
							}`;
		...
		if (play) {
			...
			getDownloadUrl(id)
				.then(url => {
					...
					request(url)
						.pipe(
							fs.createWriteStream(path.join(musicPath, filename))
						)
						.on("finish", () => {
