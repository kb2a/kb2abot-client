...
module.exports = class Rank extends Command {
	constructor() {
		super({
			keywords: "rank",
			help:
				"[--global | -g] [--id=<memberID> | -i <memberID>] [--upward | -u] [--item=<itemName>]",
			description:
				"Hiển thị rank hiện tại của 1 cá nhân hoặc tất cả rank của mọi người " +
				" theo item mà bạn muốn xếp hạng (mặc định là messagesCount)"
		});
	}
	...
	execute(args, api, parent, mssg, group) {
		...
		const sortItem = item ? item : "messagesCount";
		...
		if (global) {
			...
			for (let i = 0; i < length; i++) {
				const {name} = listMember[i];
				if (!listMember[i].hasOwnProperty(sortItem)) return;
				replyMsg += `Top ${i + 1} là: ${name} (${
					listMember[i][sortItem]
				}) ${os.EOL}`;
			}
		...
		if (id) {
			let rank = group.memberManager.find({id}, {returnIndex: true});
			if (rank == -1) rank = "không có";
			else rank++;
			const name = group.memberManager.find({id}).name;
