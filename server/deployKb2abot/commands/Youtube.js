...
module.exports = class Youtube extends Command {
	constructor() {
		super({
			keywords: ["yt", "youtube"],
			help:
				"[--play=<videoID> | -p <videoID>] [--search=<videoName> | -s <videoName>] [--autodelete=<boolean>]",
		...
		if (id) {
			...
			YD.on("finished", (err, data) => {
				...
				if (fileSize > 100) {
					...
					const replyMsg = `Tiêu đề: ${videoTitle}${
						os.EOL
					}Nhạc sĩ: ${artist}${os.EOL}Get nhạc trong ${timeGet}s${
						os.EOL
					}Dung lượng: ${round(
			...
			YD.on("error", error => {
				const replyMsg = `Đã gặp lỗi '${JSON.stringify(
					error
				)}' với id: ${id}`;
				api.sendMessage(replyMsg, mssg.threadID);
				log(
		...
		if (search) {
			ytSearch(search, (err, r) => {
				...
				for (let i = 0; i < length; i++) {
					...
					replyMsg += `${i + 1}. Tiêu đề: ${title}, id: ${videoId}${
						os.EOL
					}`;
				}
				api.sendMessage(replyMsg, mssg.threadID);
